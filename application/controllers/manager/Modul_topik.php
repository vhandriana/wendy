...
class Modul_topik extends Member_Controller {
    ...
    function hapus_daftar_topik(){
		...
		if($this->form_validation->run() == TRUE){
			$kategori_id = $this->input->post('edit-topik-id', TRUE);
			$error_hapus = 0;
			foreach( $kategori_id as $kunci => $isi ) {
				if($isi=="on"){
					if($this->cbt_tes_topik_set_model->count_by_kolom('tset_topik_id', $kunci)->row()->hasil>0){
	            		...
	            		$this->cbt_topik_model->delete('kategori_id', $kunci);
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){//hapus
            	if($this->cbt_tes_topik_set_model->count_by_kolom('tset_topik_id', $id)->row()->hasil>0){
            		...
            		// hapus topik di database
            		$this->cbt_topik_model->delete('kategori_id', $id);
                ...
                if($topik_asli!=$data['topik_nama']){
                	if($this->cbt_topik_model->count_by_kolom('topik_nama', $data['topik_nama'])->row()->hasil>0){
						...
						$this->cbt_topik_model->update('kategori_id', $id, $data);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
			...
			$jml_soal = $this->cbt_soal_model->count_by_kolom('soal_topik_id', $temp->kategori_id)->row()->hasil;
            ...
            $record[] = '<a onclick="edit(\''.$temp->kategori_id.'\')" style="cursor: pointer;" class="btn btn-default btn-xs">Edit</a>';
            $record[] = '<input type="checkbox" name="edit-topik-id['.$temp->kategori_id.']" >';

...
class Tes_hasil extends Member_Controller {
    ...
    public function index($page=null, $id=null){
        ...
        if($query_group->num_rows()>0){
        	...
        	foreach ($query_group as $temp) {
        		$select = $select.'<option value="'.$temp->lowongan_id.'">'.$temp->posisi.'</option>';
        	}
    ...
    function export($tes_id=null, $lowongan_id=null, $waktu=null, $urutkan=null){
        if(!empty($tes_id) AND !empty($lowongan_id) AND !empty($waktu) AND !empty($urutkan)){
            $this->load->library('excel');
            $waktu =  urldecode($waktu);
            $tanggal = explode(" - ", $waktu);

            $query = $this->cbt_tes_user_model->get_by_tes_group_urut_tanggal($tes_id, $lowongan_id, $urutkan, $tanggal);
            ...
            if($query->num_rows()>0){
                ...
                foreach ($query as $temp) {
                    ...
                    $worksheet->setCellValueByColumnAndRow(5, $row, $temp->posisi);
    ...
    function get_datatable(){
		...
		$lowongan_id = $this->input->get('group');
		...
		$query = $this->cbt_tes_user_model->get_datatable($start, $rows, $tes_id, $lowongan_id, $urutkan, $tanggal);
		$iFilteredTotal = $query->num_rows();
		
		$iTotal= $this->cbt_tes_user_model->get_datatable_count($tes_id, $lowongan_id, $urutkan, $tanggal)->row()->hasil;

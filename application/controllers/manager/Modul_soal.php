	        	...
	        	foreach ($user_topik as $kategori_id) {
	        		...
	        		if($query_topik->num_rows()>0){
	        			...
	        			if(!empty($data['data_topik'])){
        					if($data['data_topik']==$topik->kategori_id){
        						$select = $select.'<option value="'.$topik->kategori_id.'" selected>'.$topik->modul_nama.' - '.$topik->topik_nama.'</option>';
        					}else{
        						$select = $select.'<option value="'.$topik->kategori_id.'">'.$topik->modul_nama.' - '.$topik->topik_nama.'</option>';	
        					}
        				}else{
        					$select = $select.'<option value="'.$topik->kategori_id.'">'.$topik->modul_nama.' - '.$topik->topik_nama.'</option>';
        				}

...
class Modul_daftar extends Member_Controller {
    ...
    public function index(){
        ...
        if($query_user->num_rows()>0){
            ...
            if(!empty($query_user->opsi1)){
                ...
                foreach ($user_topik as $kategori_id) {
                    $query_topik = $this->cbt_topik_model->get_by_kolom_join_modul('kategori_id', $kategori_id);
                    if($query_topik->num_rows()>0){
                        $topik = $query_topik->row();
                        
                        $jml_soal = $this->cbt_soal_model->count_by_kolom('soal_topik_id', $topik->kategori_id)->row()->hasil;
                        $counter++;
                        $select = $select.'<option value="'.$topik->kategori_id.'">'.$topik->modul_nama.' - '.$topik->topik_nama.' ['.$jml_soal.']</option>';
                    }
                }
            }else{
	...
	function cetak_soal($kategori_id=null){
		if(!empty($kategori_id)){
			$query_soal = $this->cbt_soal_model->get_by_kolom('soal_topik_id', $kategori_id);
			if($query_soal->num_rows()>0){
				$query_soal = $query_soal->result();

				$query_topik = $this->cbt_topik_model->get_by_kolom_limit('kategori_id', $kategori_id, 1)->row();
		...
		if(!empty($modul_id)){
			...
			if($query->num_rows()>0){
				...
				foreach ($query as $temp) {
					$data['select'] = $data['select'].'<option value="'.$temp->kategori_id.'">'.$temp->topik_nama.'</option>';	
				}
    ...
    function get_by_id($id=null){
    	$data['data'] = 0;
		if(!empty($id)){
			$query = $this->cbt_topik_model->get_by_kolom('kategori_id', $id);
			if($query->num_rows()>0){
				$query = $query->row();
				$data['data'] = 1;
				$data['id'] = $query->kategori_id;

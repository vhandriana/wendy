...
class Tool_exportimport_soal extends Member_Controller {
    ...
    public function exportsoal(){
		...
		if($query_topik->num_rows()>0){
			...
			foreach ($query_topik as $topik) {
				$sql = $sql.'INSERT INTO `kategori` (`kategori_id`, `topik_modul_id`, `topik_nama`, `topik_detail`, `topik_aktif`) VALUES (\''.$topik->kategori_id.'\', \''.$topik->topik_modul_id.'\', \''.addslashes($topik->topik_nama).'\', \''.addslashes($topik->topik_detail).'\', '.$topik->topik_aktif.')'.$delimiter;
			}			
		}
    ...
    public function exportsoal(){
		...
		$prefs = array(
	        'tables'        => array('cbt_modul', 'kategori', 'cbt_soal', 'cbt_jawaban'),
	        'ignore'        => array(),
	        'format'        => 'txt',

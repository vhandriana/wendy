...
class Daftar_pelamar extends Member_Controller {
	private $kode_menu = 'daftar_pelamar';
	private $kelompok = 'peserta';
	private $url = 'manager/daftar_pelamar';
	
    function __construct(){
		parent:: __construct();
		$this->load->model('cbt_user_grup_model');
		$this->load->model('cbt_lowongan_model');
    ...
    public function index(){
        ...
        $this->template->display_admin($this->kelompok.'/peserta_daftar_view2', 'Daftar Peserta', $data);
    }

     public function detail_data_lowongan($idLowongn=null){
       ...
       $this->load->model('M_det_lowongan', 'info');
		...
		$data['lwngan'] = $this->info->get_data_lowongan($idLowongn);

		
		

        
        $this->template->display_admin($this->kelompok.'/peserta_daftar_view1', 'Daftar Peserta', $data);
    }
    ...
    public function detail_pelamar($iduser)
	{
		...
		$data['user'] = $this->db->get_where('user1', ['id' => $iduser])->row_array();

		$this->load->model('M_det_lowongan', 'info');
		$idUser2= $this->info->get_iduser2($iduser)->row()->user2_id;
		var_dump($idUser2);
		$data['user2'] = $this->info->get_det_data_login_ujian($idUser2);

		
	$this->template->display_admin($this->kelompok.'/peserta_daftar_detail', 'Daftar Peserta', $data);
	}

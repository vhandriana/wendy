    ...
    function import_file($inputfile){
        ...
        if($highestRow>2){
            ...
            while($kosong<4){
                ...
                if($kosong==0){
                    if($this->cbt_user_grup_model->count_by_kolom('posisi', $kolom5)->row()->hasil>0){
                    	if($this->cbt_user_model->count_by_kolom('user_name', $kolom1)->row()->hasil>0){
				            ...
				            $data['user_lowongan_id'] = $this->cbt_user_grup_model->get_by_kolom_limit('posisi', $kolom5, 1)->row()->lowongan_id;

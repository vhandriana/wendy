...
class Tes_tambah extends Member_Controller {
    ...
    public function index($tes_id=null){
        ...
        if($query_group->num_rows()>0){
        	...
        	foreach ($query_group as $temp) {
        		if($is_edit!=0){
        			if($this->cbt_tesgrup_model->count_by_tes_and_group($tes_id, $temp->lowongan_id)->row()->hasil>0){
        				$select = $select.'<option value="'.$temp->lowongan_id.'" selected>'.$temp->posisi.'</option>';
        			}else{
        ...
        if(!empty($modul)){
            ...
            if($query_topik->num_rows()){
                ...
                foreach ($query_topik as $topik) {
                    ...
                    $select = $select.'<option value="'.$topik->kategori_id.'">'.$topik->topik_nama.' ['.$jml_soal.' soal]</option>';
                }
    ...
    function get_datatable_soal(){
	    ...
	    foreach ($query as $temp) {			
			...
			$query_topik = $this->cbt_topik_model->get_by_kolom_limit('kategori_id', $temp->tset_topik_id, 1)->row();

...
class Peserta_daftar extends Member_Controller {
    ...
    function tambah(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $data['user_lowongan_id'] = $this->input->post('tambah-group', true);

            if($this->cbt_user_grup_model->count_by_kolom('lowongan_id', $data['user_lowongan_id'])->row()->hasil>0){
    ...
    function get_by_id($id=null){
		...
		if(!empty($id)){
			...
			if($query->num_rows()>0){
				...
				$data['group'] = $query->user_lowongan_id;
			}
		}
    ...
    function edit(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            if($pilihan=='hapus'){//hapus
                ...
                $data['user_lowongan_id'] = $this->input->post('edit-group', true);
    ...
    function get_datatable(){
	    ...
	    foreach ($query as $temp) {			
            ...
            $query_group = $this->cbt_user_grup_model->get_by_kolom_limit('lowongan_id', $temp->user_lowongan_id, 1)->row();
            $record[] = $query_group->posisi;

			
            // $record[] = '<a onclick="edit(\''.$temp->user_id.'\')" style="cursor: pointer;" class="btn btn-default btn-xs">Detail</a>';

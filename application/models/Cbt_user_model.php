...
class Cbt_user_model extends CI_Model{
	...
	function get_by_kolom($kolom, $isi){
        $this->db->select('user_id,user_lowongan_id,user_name,user_password,user_firstname,user_regdate')
                 ->where($kolom, $isi)
                 ->from($this->table);
        return $this->db->get();
    }
    ...
    function get_by_username($username){
        $this->db->join('lowongan', 'user2.user_lowongan_id = lowongan.lowongan_id')
	...
	function get_datatable($start, $rows, $kolom, $isi, $group){
        $query = '';
        if($group!='semua'){
            $query = 'AND user_lowongan_id='.$group;
        }

...
class Cbt_tes_user_model extends CI_Model{
    ...
    function get_by_tes_group_urut_tanggal($tes_id, $lowongan_id, $urutkan, $tanggal){
        $sql = '';
        if($tes_id!='semua'){
            $sql = ' AND tesuser_tes_id="'.$tes_id.'"';
        }
        if($lowongan_id!='semua'){
            $sql = $sql.' AND user_lowongan_id="'.$lowongan_id.'"';
        }
        $order = '';
        ...
        $this->db->select('cbt_tes_user.*, cbt_tes.*, user2.*, lowongan.posisi, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai ')
                 ->where('(tesuser_creation_time>="'.$tanggal[0].'" AND tesuser_creation_time<="'.$tanggal[1].'" '.$sql.' )')
                 ->from($this->table)
                 ->join('user2', 'cbt_tes_user.tesuser_user_id = user2.user_id')
                 ->join('lowongan', 'user2.user_lowongan_id = lowongan.lowongan_id')
                 ->join('cbt_tes', 'cbt_tes_user.tesuser_tes_id = cbt_tes.tes_id')
	...
	function get_datatable($start, $rows, $tes_id, $lowongan_id, $urutkan, $tanggal){
        $sql = '';
        if($tes_id!='semua'){
            $sql = ' AND tesuser_tes_id="'.$tes_id.'"';
        }
		...
		$this->db->select('cbt_tes_user.*,lowongan.posisi, cbt_tes.*, user2.*, SUM(`cbt_tes_soal`.`tessoal_nilai`) AS nilai ')
                 ->where('(tesuser_creation_time>="'.$tanggal[0].'" AND tesuser_creation_time<="'.$tanggal[1].'" '.$sql.' )')
                 ->from($this->table)
                 ->join('user2', 'cbt_tes_user.tesuser_user_id = user2.user_id')
                 ->join('lowongan', 'user2.user_lowongan_id = lowongan.lowongan_id')
    ...
    function get_datatable_count($tes_id, $lowongan_id, $urutkan, $tanggal){
        $sql = '';
        if($tes_id!='semua'){
            $sql = ' AND tesuser_tes_id="'.$tes_id.'"';
        }
